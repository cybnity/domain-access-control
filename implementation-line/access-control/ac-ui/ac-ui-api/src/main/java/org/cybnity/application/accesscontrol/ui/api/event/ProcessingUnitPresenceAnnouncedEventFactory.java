...
public class ProcessingUnitPresenceAnnouncedEventFactory {
     ...
     */
    public ProcessingUnitPresenceAnnounced create(Map<DomainEventType, UICapabilityChannel> supportedEventTypesToRoutingPath, String puServiceName, EntityReference priorEventRef) throws IllegalArgumentException {
        ...
        for (Map.Entry<DomainEventType, UICapabilityChannel> route : supportedEventTypesToRoutingPath.entrySet()) {
        ...
        if (puServiceName != null && !puServiceName.isEmpty())
            announced.appendSpecification(new Attribute(AttributeName.ServiceName.name(), puServiceName));
